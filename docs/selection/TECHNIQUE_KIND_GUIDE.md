...
The first kind-axis pass established `domain` as the ownership and first-routing
axis, with `kind` as the narrow second selector. It intentionally kept `family`
optional and scout-only, left `tags` freeform, and bounded the adoption pass so
the corpus could gain `kind` without turning the axis into a new policy layer.
