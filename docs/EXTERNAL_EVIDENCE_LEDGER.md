...
|---|---|---|---|---|
| [AOA-T-0032](../techniques/evaluation/context-report-for-ci/TECHNIQUE.md) | repo-local drift and evaluation-report lane plus public agent-markdown CI check/report lane | local drift reports and composition audits; public instruction-check and workflow-report surfaces such as Continue `/check` and GitHub Agentic Workflows daily repo reports that emit PR checks or activity summaries instead of the same CI-facing composition coverage/drift artifact | one second public CI-facing report over context composition coverage or token-drift that stays separate from composition mechanics, remediation policy, and generic PR policy checks | search public CI or docs-validation repos where assembled prompt/context coverage and token drift are emitted as a read-only artifact, not just as pass/fail checks or activity digests |
| [AOA-T-0026](../techniques/history/session-capture-as-repo-artifact/TECHNIQUE.md) | repo-local witness, checkpoint, and history-adjacent lane plus public local session-store products | witness traces, transcript packaging, memory or recall systems, cloud-history wrappers, proposal-only surfaces such as OpenAI Codex issue `#2765`, and GitHub Copilot CLI session-state storage under `~/.copilot/session-state/` plus a local SQLite session store that powers resume, `/chronicle`, and history Q&A rather than project-scoped repo artifacts | one second public repo-local session capture surface where history is kept as a reviewable project artifact rather than a memory product or home-directory session store | search public repos and tools that write AI session history into versionable workspace directories or repo-visible artifacts without adding search, recall, or instruction authority |
| [AOA-T-0036](../techniques/agent-workflows/render-truth-before-startup/TECHNIQUE.md) | repo-local runtime and lifecycle lane plus public compose prestart-render lane | lifecycle wrappers, readiness-only checks, config rendering without a real pre-start review seam, `OpenDAX`-style config-before-startup surfaces, and CoreMedia's public `docker compose config --services` prestart step backed by Docker Compose config docs; the lane proves a helpful service preview before startup, but not yet a distinct render-review contract over effective runtime truth | one second public runtime surface where effective composed truth is rendered and explicitly reviewed before startup as its own safety seam | search operator repos where render output is a named pre-start review step over actual resolved runtime state, not just a command example before `up` |
