...
- Use when: the repo publishes generated files or snapshots that depend on sibling repos or CI or release checks validate...
- Avoid when: the generated outputs are fully single-repo and CI reads only the current or the task is ordinary code...
- Needs: one owner repo that is about to publish generated outputs; one explicit list of workflow-pinned sibling refs...
- Produces: one local validation pass that matches the workflow-pinned matrix closely; one generated diff or clean-state result produced...
...
- Use when: a shared practice pattern has already been noticed, approved upstream, or or a local repository, team, or owner...
...
- Produces: one local adoption gate result such as adopt, shadow, quarantine,; one owner-consent statement or missing-consent blocker; one...
...
- Use when: a reusable technique or technique candidate is repeatedly needed as part of or an adoption review shows that...
...
- Produces: one retention verdict such as retain, revise, quarantine, defer, or; a current evidence note; a drift or...
...
- Use when: a retention review says an adopted or shadowed practice should route toward or a replacement, merge target, reanchor...
...
- Produces: one obsolescence route packet; one route label such as supersede, merge, reanchor, defer, drop, or; the owner...
...
- Use when: one claim, decision, generated artifact, route, or review state is already in or the current blocker is a...
- Avoid when: the real task is to prove correctness, score quality, or issue an evaluation or the review surface is...
