...
- Use when: one repeated reviewed quest unit is already isolated from the wider session family or the remaining question is...
...
- Use when: one chosen owner or promotion target already looks plausible, but at least one adjacent target also feels tempting...
...
- Use when: work starts from a sibling workspace that contains several related repositories under one root or the next step...
- Avoid when: the task is truly single-repo and no meaningful workspace-root posture exists or the main need is just one...
- Needs: one workspace root; one primary repo root or one explicit reason to keep the workspace root primary...
- Produces: one explicit ingress report for the current workspace and repo root; one explicit guard report before risky...
...
- Use when: a reviewed audit, review packet, or bounded findings set already exists or the route must prove that each...
- Avoid when: the source findings are still raw, unreviewed, or too vague to bound or the work is a generic...
- Needs: one reviewed audit artifact or bounded findings set; live source checkouts for the owning repositories; one owner...
- Produces: one live-confirmed finding map; one bounded owner-surface fix slice per finding or cluster; one targeted regression proof...
...
- Use when: a control plane recommends skills, capabilities, or bounded workflow objects from a corpus that may be larger than...
- Avoid when: every recommended object is always bundled with the same runtime and host executability can never diverge or the...
- Needs: one router recommendation result such as activate_now, must_confirm, and suggest_next; one explicit host inventory source or one...
- Produces: one annotated recommendation report where each item keeps both its routing reason and its host-availability state; one...
...
- Use when: the final owning repository is intentionally GitHub-only in the current route or the coordination layer can plan and...
- Avoid when: a real local source checkout exists and is the intended execution surface or the route still needs broader...
- Needs: one bounded owner-side goal with explicit non-goals; one coordination or seed-garden surface that currently tracks the work...
- Produces: one merged owner-side landing in the real repository; one visible GitHub trail such as issue, milestone, and...
...
- Main risk: the coordination layer updates its own staging note before the owner PR is really merged.
- Validate by: the owner-side goal, non-goals, and stop condition were explicit before the PR landed; the owner repository merged the bounded...
...
- Use when: the repo publishes generated files or snapshots that depend on sibling repos or other bounded upstream surfaces or...
- Avoid when: the generated outputs are fully single-repo and CI reads only the current checkout or the task is ordinary...
- Needs: one owner repo that is about to publish generated outputs; one explicit list of workflow-pinned sibling refs...
- Produces: one local validation pass that matches the workflow-pinned matrix closely enough to predict CI honestly; one generated...
...
- Main risk: local rebuilds go green against workspace state while CI still reads older or pinned sibling refs and fails.
...
- Use when: a shared practice pattern has already been noticed, approved upstream, or proven useful elsewhere or a local repository...
- Avoid when: the source material is still several mixed patterns that have not been split or the real task is...
- Needs: one shared pattern proposed for local adoption; the local owner or surface that would change behavior; source...
- Produces: one local adoption gate result such as adopt, shadow, quarantine, defer, or reject; one owner-consent statement or...
...
- Use when: a reusable technique or technique candidate is repeatedly needed as part of agent-facing execution or an adoption review...
- Avoid when: the source material is still several unrelated practices or workflow needs or the real task is choosing whether...
- Needs: one technique, technique candidate, or adopted practice under review; the execution pressure that suggests skill packaging may...
- Produces: one skill-proposal handoff packet; explicit technique dependency references; one proposed skill trigger boundary.
...
- Use when: a practice was previously adopted, shadowed, or made part of normal behavior or the adoption record included a...
- Avoid when: the practice has not passed a local adoption gate yet or the real question is whether to adopt...
- Needs: one adopted or shadowed practice under review; the original adoption note or equivalent owner record; current evidence...
- Produces: one retention verdict such as retain, revise, quarantine, defer, or route_to_obsolescence; a current evidence note; a drift...
...
- Use when: a retention review says an adopted or shadowed practice should route toward obsolescence instead of staying active or...
- Avoid when: the practice has not passed a local adoption gate or shadow-use review yet or the only question is...
- Needs: one adopted or shadowed practice under obsolescence pressure; the current stage or status of that practice; the...
- Produces: one obsolescence route packet; one route label such as supersede, merge, reanchor, defer, drop, or deprecation_review; the...
- Core contract: one route packet covers one adopted or shadowed practice and one owner receipt target; obsolescence is not erasure.
...
- Use when: one claim, decision, generated artifact, route, or review state is already in view or the current blocker is...
- Avoid when: the real task is to prove correctness, score quality, or issue an evaluation verdict or the review surface...
- Needs: one reviewed claim, decision, artifact, or route state; the current review state or evidence gap; one missing...
- Produces: one evidence request naming the reviewed claim or decision; one concrete missing evidence object; one short reason...
...
- Use when: one claim, assertion, generated summary, route statement, or review sentence is already in view or the claim may...
...
- Use when: markdown-first repositories that need a small machine-readable layer for lookup or routing or corpora where domain, kind, status...
...
- Use when: one shared schema, contract, or vocabulary is consumed across several repositories or a neighboring repository needs a local...
- Avoid when: every repository can depend directly on the canonical contract and no local mirror is needed or two repositories...
- Needs: one shared contract that several repositories already consume; one explicit canonical owner for that contract; zero or...
- Produces: one explicit canonical contract home; zero or more legal mirrors that stay subordinate to the canonical source...
...
- Use when: one claim, decision, review state, or documentation statement is already in view or one available source, line, artifact...
- Avoid when: the real task is to prove correctness, score adequacy, or issue an evaluation verdict or the review needs...
...
- Use when: one helper, UI, worker, or edge service needs to stop while shared data services should stay available or...
- Avoid when: the target service owns exclusive mutable state that must be quiesced together with its dependencies or the shared...
- Needs: one named target service; one named shared substrate set that should remain alive; one stop handle for...
- Produces: one stopped target service; one explicit continuity check for the shared substrate; one bounded closeout or recovery...
