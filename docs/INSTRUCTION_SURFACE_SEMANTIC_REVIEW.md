...
This review-only pilot checks whether the current instruction-surface cluster still reads as three distinct techniques rather than one blurred "managed generated agent docs" package.

Why this cluster was chosen now:

- the cluster now mixes one canonical composition technique with two promoted adjacent imports, so semantic clarity matters before any future status review
- all three techniques govern generated or managed instruction-facing documentation surfaces, which makes seam drift easy to miss if left implicit
...
- The strongest watch seam is that future wording must keep source cardinality, source ownership, and output shape explicit so the cluster does not blur into generic "managed generated agent docs."
...
Concrete example strengthening has already landed for all three techniques, `ruler` is now explicitly locked as the bounded external origin for `AOA-T-0013`, and `n-skills` now lands as the adjacent provenance-mirroring import `AOA-T-0024`. The next step should therefore move to one more independent live instruction-distribution context for `AOA-T-0013` rather than more example-only reinforcement or another import-only review.
