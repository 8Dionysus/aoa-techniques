...
- Invariant boundary: [AOA-T-0004](../techniques/execution/intent-chain/intent-plan-dry-run-contract-chain/TECHNIQUE.md) owns artifact-first intent normalization and dry-run contract checking before the live path is trusted. [AOA-T-0014](../techniques/agent-workflows/tdd-slice/TECHNIQUE.md) owns the execution discipline for one bounded implementation slice after the target path is already clear enough to work on directly.
