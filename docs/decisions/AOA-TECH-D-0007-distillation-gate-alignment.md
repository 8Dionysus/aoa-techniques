# Distillation Gate Alignment

Status: accepted
Date: 2026-05-01

## Index Metadata

- Decision ID: AOA-TECH-D-0007
- Original date: 2026-05-01
- Surface classes: mechanic package
- Technique axes: mechanic bridge, source-lift
- Mechanic parents: distillation
- Guard families: mechanic topology
- Posture: accepted

## Context

Distillation now has active part homes, external and cross-layer candidate
registries, and recurrence observation that reads cross-layer candidate evidence
without gaining candidate authority. The project direction also clarified that
techniques should become many compact, portable, atomic moves rather than broad
mini-skills.

That left one workflow risk: donor refinery, external import, and candidate
ledger reopen rules could each ask for similar information in different words.
Future agents might therefore draft a bundle before the candidate has a clear
atom/topology shape or before owner boundary, local route, and stop line are
clear.

## Options

- Keep the existing softer wording and rely on reviewers to reconstruct the
  packet during import.
- Move all classification responsibility into generated registries.
- Add one explicit gate packet to the active Distillation surfaces while keeping
  registries as evidence and bundle review as the promotion path.

## Decision

Align donor refinery, external import, and candidate-ledger reopen rules around
one Distillation gate packet.

The packet has two sides:

- atom/topology: atomic move, current atomic status, likely `domain`, primary
  `kind`, family posture, capability class, substrate, execution profile, risk
  posture, and nearest relation or overlap
- boundary/portability: source owner or boundary, local route, stop line,
  portable core, AoA-only context, and donor exclusions

Candidates that cannot name the packet stay in their ledger, overlap,
incubation, or not-yet-technique-shaped lane. The generated registries remain
evidence only; they do not promote candidates or authorize bundle drafting.

## Consequences

This adds a small amount of intake friction before the next import wave, but it
keeps future work honest: a candidate has to be one portable move before it can
become a technique bundle.

The gate also preserves standalone use of `aoa-techniques`. External users can
adopt a technique without deploying OS Abyss, while OS Abyss can still keep
local integration context around the same candidate.
