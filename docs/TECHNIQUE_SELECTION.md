...
| [AOA-T-0053](../techniques/history/local-first-session-index/TECHNIQUE.md) | `promoted` | `source_backed` | `bounded` | Build a local searchable index over already-saved session artifacts so teams can browse or query saved history without reopening capture semantics or turning the index into memory or dashboard doctrine. |
...
- `export_ready` is currently `true` for 69/69 techniques.
