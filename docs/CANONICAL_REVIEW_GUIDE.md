...
If the question is still which `promoted` bundle is honestly closest to review, or what proof is still missing before `approve for canonical promotion` becomes real, start with [Promotion Readiness Matrix](../mechanics/audit/PROMOTION_READINESS_MATRIX.md) instead.

If the question is how to run the current evidence-prep swarm without widening bundle meaning or faking closure, start with [Promotion Wave A Runbook](../mechanics/audit/PROMOTION_WAVE_A_RUNBOOK.md) instead.
