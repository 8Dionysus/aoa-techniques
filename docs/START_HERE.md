...
- open [Roadmap](../ROADMAP.md) when the question is which repo-level direction, horizon, or future trigger should shape the next change
- open [Questbook](../QUESTBOOK.md) when the question is which durable technique-canon obligation should survive the current diff
- open [External Technique Candidates](../mechanics/distillation/parts/external-candidate-ledger/README.md) when the question is which remaining external seed idea is actually worth distilling into a future bundle
- open [Cross-Layer Technique Candidates](../mechanics/distillation/parts/cross-layer-candidate-ledger/README.md) when the question is how the Dionysus donor-note candidate set should be filtered without widening the external-only intake surface
