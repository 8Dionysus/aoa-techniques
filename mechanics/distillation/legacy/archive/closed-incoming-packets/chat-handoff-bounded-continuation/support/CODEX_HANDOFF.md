# Codex Handoff - Chat Handoff Bounded Continuation

You are operating inside `aoa-techniques` under explicit human control.

## Your job

Use the evidence surfaces in `mechanics/distillation/legacy/archive/closed-incoming-packets/chat-handoff-bounded-continuation/` to
inspect landed Wave 3 provenance and the explicit exclusion. The first-pass
landing queue is closed.

## Read first

1. `README.md` at repo root
2. `mechanics/distillation/parts/donor-refinery/README.md`
3. `mechanics/distillation/parts/external-import-runbook/README.md`
4. `TECHNIQUE_INDEX.md`
5. `mechanics/distillation/legacy/archive/closed-incoming-packets/chat-handoff-bounded-continuation/docs/EXTERNAL_TECHNIQUE_CANDIDATES_CHAT_HANDOFF_BOUNDED_CONTINUATION.md`
6. `mechanics/distillation/legacy/archive/closed-incoming-packets/chat-handoff-bounded-continuation/docs/CHAT_HANDOFF_BOUNDED_CONTINUATION_PLANTING_ORDER.md`

## Operating posture

- treat this packet as evidence, not merge authority
- do not recreate packet-local `candidate_bundles/**` for already landed techniques
- do not edit `TECHNIQUE_INDEX.md`, `generated/**`, or repo-wide queue docs
- keep governed actions and generic phase-sync out of this wave
- treat `governed-action-surfaces` as closed for this packet; future governed-action work needs a separate intake

## Desired worker output

Always answer in this shape:

- candidate chosen
- overlap watch
- boundary statement
- what stays out
- files touched or proposed
- whether a new intake is justified
