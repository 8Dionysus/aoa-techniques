...
- current promoted corpus: `34` techniques
- matrix categorization status: `34` promoted techniques are explicitly categorized in the pack matrix below; Wave 0 matrix expansion is closed for `AOA-T-0075` through `AOA-T-0107`
...
- prior graduation wave: [AOA-T-0075](../../../../techniques/continuity/donor-harvest/session-donor-harvest/TECHNIQUE.md) and [AOA-T-0077](../../../../techniques/continuity/donor-harvest/harvest-packet-contract/TECHNIQUE.md) moved to `canonical` on 2026-05-12 after exact-fit live reinforcement from `aoa-sdk`'s checkpoint-closeout bridge: a reviewed artifact plus reviewed checkpoint carry can produce a bounded `HARVEST_PACKET.json` with accepted and deferred candidates, evidence density, extract counts, owner-layer distribution, and checkpoint-review carry; `HARVEST_PACKET_RECEIPT.json` and core skill receipts preserve the handoff; and the closeout API reads accepted candidates into owner follow-through briefs without letting the packet become memory canon, routing authority, or final owner acceptance. LangSmith annotation queues and dataset docs were checked as supporting public reviewed-run curation evidence, not primary proof, while checkpoint capture, transcript packaging, local `.aoa` storage, exact SDK command wrappers, memory writeback, stats refresh, owner placement, progression, quest promotion, evaluation dataset governance, and final promotion verdicts stay outside the bundles
- prior graduation wave: [AOA-T-0074](../../../../techniques/ingest/media-ingest/telegram-export-normalization-to-local-store/TECHNIQUE.md) moved to `canonical` on 2026-05-12 after exact-fit public reinforcement from `3bl3gamer/tg_history_dumper`: messages are saved as local JSON Lines, media files stay linked to chat and message ids, related users and chats remain visible as JSONL peer surfaces, the last saved message id drives incremental continuation, interrupted file downloads can resume, and account/auth/session dumps remain optional adjacent surfaces rather than the normalized message-store contract. `GeiserX/Telegram-Archive`, `jackwener/tg-cli`, `groupultra/telegram-search`, HTML/CSV/Markdown converters, and marketing/member-scraper projects were checked as wider, adjacent, lossy, or out of scope, while auth bootstrap, session conversion, account/session dumps, live control, archive presentation, search products, deletion/edit sync, routing, recall, and memory writeback stay outside the bundle
- prior graduation wave: [AOA-T-0073](../../../../techniques/ingest/media-ingest/semantic-media-bucketing-with-vision-plus-ocr/TECHNIQUE.md) moved to `canonical` on 2026-05-12 after exact-fit public reinforcement from `end1989/ai-image-classification`: configured mixed-media labels stay explicit, CLIP image embeddings score those labels, OCR text and confidence stay a separate side result, OCR text only boosts text-heavy labels such as receipt, chat, and work, review and auto-move thresholds remain distinct, user corrections remain visible, and file actions plus undo stay outside classification. `chintan-projects/photo-triage-agent`, `Aditya-Vasipalli/screensort` / Fragmenta, receipt-only extractors, and broad cleanup products were checked as adjacent or too wide, while OCR pipeline ownership, duplicate grouping, moderation or NSFW policy, face/person identification, deletion, archive/move policy, UI workflows, database schemas, model-serving detail, and full media-management behavior stay outside the bundle
- prior graduation wave: [AOA-T-0072](../../../../techniques/ingest/media-ingest/perceptual-media-dedupe-with-threshold-review/TECHNIQUE.md) moved to `canonical` on 2026-05-12 after exact-fit public reinforcement from `qarmin/czkawka`: Similar Images finds visually similar rather than byte-identical images, `max_difference` keeps threshold strictness explicit, perceptual hash algorithm and hash size remain tunable, grouped results carry difference-derived similarity labels and can be printed or saved as JSON, and deletion stays a separate default-off option. Czkawka's relevant core and CLI surfaces are MIT licensed, while mixed-license GUI/assets/product layers, delete methods, hardlink strategy, cache layout, video/music tools, semantic taxonomy, archive policy, quality ranking, and full media-management behavior stay outside the bundle
- prior graduation wave: [AOA-T-0071](../../../../techniques/ingest/media-ingest/template-backed-field-extraction-after-ocr/TECHNIQUE.md) moved to `canonical` on 2026-05-12 after exact-fit public reinforcement from `kotaro-kinoshita/yomitoku`: YAML schemas define explicit field targets, rule-based extraction uses visible `cell_id`, `bbox`, `description`, and `regex` methods, JSON output preserves normalized value, raw text, source, cell ids, bounding boxes, and confidence, and misses remain visible through `source: not_found` plus low confidence. `codebywiam/invoice-ocr` and `nzregs/receipt-api` were checked as adjacent regex/heuristic extraction lanes, not primary proof, while OCR engines, layout analyzers, LLM extraction services, schema products, locale policy, bookkeeping flows, and total document-understanding stacks stay outside the bundle
- prior graduation wave: [AOA-T-0070](../../../../techniques/ingest/media-ingest/two-stage-document-ocr-pipeline/TECHNIQUE.md) moved to `canonical` on 2026-05-12 after exact-fit public reinforcement from `JaidedAI/EasyOCR`: public OCR results preserve bounding boxes, recognized text, and confidence; `readtext()` calls a separate detection method before recognition; dictionary and JSON modes preserve boxes, text, and confidence as a structured result surface; and detection plus recognition model concerns remain separately visible, while OCRmyPDF, Tesseract.js, Surya, PaddleOCR, and docTR stay out of the primary proof as searchable-PDF, engine-packaging, full document-understanding, or donor-family lanes. Serving, training, benchmark doctrine, searchable-PDF generation, receipt schema law, template extraction, semantic media bucketing, and automation stacks stay outside the bundle
- prior graduation wave: [AOA-T-0069](../../../../techniques/governance/approval-evidence/approval-bound-durable-jobs/TECHNIQUE.md) moved to `canonical` on 2026-05-12 after exact-fit public reinforcement from `pydantic/pydantic-ai`: deferred approval and external-tool calls preserve pending call identity, return `DeferredToolRequests`, gather approval or external result outside the agent run, and resume through original message history plus `DeferredToolResults`, while durable execution support covers long-running asynchronous and human-in-the-loop workflows; LangGraph interrupts support the checkpoint/thread/resume boundary, while scheduler products, queue platforms, worker-fleet governance, retry doctrine, dashboards, generic confirmation prompts, one-shot fail-closed gates, and total durable-execution platform behavior stay outside the bundle
- prior graduation wave: [AOA-T-0068](../../../../techniques/governance/approval-evidence/fail-closed-evidence-gate/TECHNIQUE.md) moved to `canonical` on 2026-05-12 after exact-fit public reinforcement from `mvar-security/clawzero`: a deterministic execution boundary sits between model output and tool or process execution, sink decisions are evaluated before commands or tool calls run, blocked decisions raise or prevent wrapped execution, and witness artifacts preserve reviewable verdict evidence, while keeping human approval, durable jobs, signed-witness infrastructure, attack packs, policy-authoring platforms, gateway products, sandboxing, compliance export, and total trust governance outside the bundle
- prior graduation wave: [AOA-T-0067](../../../../techniques/history/history-artifacts/transcript-linked-code-lineage/TECHNIQUE.md) moved to `canonical` on 2026-05-12 after exact-fit public reinforcement from `ai4curation/ai-blame`: AI agent execution traces can be turned into line-by-line blame with line number, line content, model, session id, timestamp, and agent tool metadata; reviewers can focus on a line or block, then reopen the related saved transcript through transcript listing and viewing, while keeping AI-percentage scoring, policy gates, review enforcement, transcript indexing, hosted search, dashboards, telemetry, repository analytics, and memory recall outside the bundle
- prior graduation wave: [AOA-T-0066](../../../../techniques/history/history-artifacts/transcript-replay-artifact/TECHNIQUE.md) moved to `canonical` on 2026-05-12 after exact-fit public reinforcement from `dataprofessor/cortex-replay` plus Snowflake's public Cortex Code replay guide: already-saved AI coding session transcripts are transformed into one self-contained interactive replay artifact, session selection and direct transcript inputs stay bounded to existing saved artifacts, turn/time filtering plus visibility toggles stay explicit replay-output controls, and secret redaction keeps public reuse separate from raw transcript exposure, while keeping first-save capture, transcript packaging, local indexing, witness tracing, hosted sharing, dashboards, replay editors, memory doctrine, and replay-as-proof claims outside the bundle
- prior graduation wave: [AOA-T-0065](../../../../techniques/tool-use/tool-gateway/mcp-gateway-proxy/TECHNIQUE.md) moved to `canonical` on 2026-05-12 after exact-fit public reinforcement from `smart-mcp-proxy/mcpproxy-go`: one MCP client endpoint fronts multiple configured upstream MCP servers, tool metadata is indexed across connected servers, tools keep server-scoped names, calls are mediated through built-in `call_tool_*` variants with intent fields, and sensitive-data inspection happens over arguments and responses at the proxy boundary, while keeping routing-mode policy, BM25 ranking, quarantine governance, Docker isolation, lifecycle management, UI/dashboard behavior, OAuth, registry publication, marketplace curation, and enterprise MCP platform doctrine outside the bundle
- prior graduation wave: [AOA-T-0064](../../../../techniques/instruction/capability-registry/capability-discovery/TECHNIQUE.md) moved to `canonical` on 2026-05-12 after exact-fit public reinforcement from Nacos's A2A Registry guide: published AgentCards can be queried by name through SDK lookup, exact HTTP detail lookup uses `namespaceId` and `agentName`, list search exposes `pageNo`, `pageSize`, `agentName`, `namespaceId`, and `search=blur`, and future skill/tag/description filters stay clearly marked as future search dimensions, while keeping entry publication, endpoint subscription, A2A invocation, ranking, trust policy, registry console/product behavior, marketplace curation, and graph semantics outside the bundle
- prior graduation wave: [AOA-T-0063](../../../../techniques/instruction/capability-registry/versioned-agent-registry-contract/TECHNIQUE.md) moved to `canonical` on 2026-05-12 after exact-fit public reinforcement from Nacos's A2A Registry guide: AgentCards are published as agent registry entries with namespace/name identity, unique versions, a current default published version, SDK and HTTP publication paths, and explicit AgentCard fields including name, description, URL, version, and protocol version, while keeping discovery ranking, fuzzy search, endpoint subscription, A2A invocation, registry console/product behavior, trust policy, and marketplace curation outside the bundle
- prior graduation wave: [AOA-T-0062](../../../../techniques/continuity/handoff-continuation/episode-bounded-agent-loop/TECHNIQUE.md) moved to `canonical` on 2026-05-12 after exact-fit public reinforcement from Cloudflare's long-running Agents guide: durable plans split long-running work into ordered steps, execute one step at a time, checkpoint and recover state, schedule the next step only after completion, mark failed steps without silent continuation, and keep re-planning plus human oversight as review boundaries, while keeping Durable Objects, Workers, schedules, fibers, Workflows, sub-agent RPC, proof gates, context compression, and total platform lifecycle governance outside the bundle
...
| `external live-adopter lane` | `3` | Already has donor intake, documentation-first adaptation, and canonical-readiness review; still needs another real adopter outside the donor repo. |
