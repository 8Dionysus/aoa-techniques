...
current `82/82` promoted technique corpus. This file is not bundle authority,
...
- this plan now targets the full current promoted matrix, not only the lead
  sprint order.

## Long-Pass Goal

Move from one exemplar evidence cycle into a complete, repeatable,
matrix-wide pass over all `82` current promoted techniques without faking
canonical readiness.

The pass succeeds only when every current promoted bundle exits with one of
these outcomes:
...
This is a `82/82` promoted-corpus plan.

Do not call the pass complete while any current promoted bundle lacks one
reviewed closeout row in the final ledger.
...
The order below covers the whole current promoted matrix. Pack names come from
`promotion-readiness-matrix/README.md`; IDs are reconciled against generated
catalog truth.
...
44. Reconcile all `82` current promoted IDs against generated catalog truth.
