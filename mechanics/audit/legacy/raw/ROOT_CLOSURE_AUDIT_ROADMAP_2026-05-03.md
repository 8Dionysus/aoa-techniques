...
This roadmap records the current repo-first closure audit for `aoa-techniques`.
...
- `review refresh` is landed: [AGENT_WORKFLOWS_CORE_SEMANTIC_REVIEW.md](docs/AGENT_WORKFLOWS_CORE_SEMANTIC_REVIEW.md) now covers `AOA-T-0001`, `AOA-T-0004`, and `AOA-T-0014`, and [SKILL_SUPPORT_SEMANTIC_REVIEW.md](docs/SKILL_SUPPORT_SEMANTIC_REVIEW.md) is now narrowed to `AOA-T-0015`, `AOA-T-0016`, and `AOA-T-0017`
...
- the remaining legacy promoted backlog slice is now staged as:
  ...
  - use [Promotion Readiness Matrix](mechanics/audit/parts/promotion-readiness-matrix/README.md) for the full `75`-bundle promoted corpus, including newer `v0.4` / session-harvest / recovery-wave packs
...
- `keep with wording follow-up`: the distinction between public-safe wording and safe generated export is present in [CANONICAL_RUBRIC.md](docs/CANONICAL_RUBRIC.md), but it is still spread across several docs rather than summarized in one hygiene-oriented place.
...
| docs canonical core | `AOA-T-0002`, `AOA-T-0009`, `AOA-T-0012`, `AOA-T-0016` | `clear with one wording watch` | [DOCS_BOUNDARY_SEMANTIC_REVIEW.md](docs/DOCS_BOUNDARY_SEMANTIC_REVIEW.md) keeps `0002` vs `0009` clear; [INSTRUCTION_SURFACE_SEMANTIC_REVIEW.md](docs/INSTRUCTION_SURFACE_SEMANTIC_REVIEW.md) keeps `0012` distinct from `0013`; [SKILL_SUPPORT_SEMANTIC_REVIEW.md](docs/SKILL_SUPPORT_SEMANTIC_REVIEW.md) explicitly watches [AOA-T-0016](techniques/docs/bounded-context-map/TECHNIQUE.md) for drift into generic architecture formalism. |
| evaluation canonical core | `AOA-T-0003`, `AOA-T-0006`, `AOA-T-0007`, `AOA-T-0008`, `AOA-T-0010`, `AOA-T-0011`, `AOA-T-0015` | `clear with two watch seams` | [PUBLISHED_SUMMARY_SEMANTIC_REVIEW.md](docs/PUBLISHED_SUMMARY_SEMANTIC_REVIEW.md) keeps `0006/0008/0010/0011` distinct and explicitly watches [AOA-T-0011](techniques/evaluation/required-vs-optional-source-rendering/TECHNIQUE.md) for package-appendix drift. [EVALUATION_CHAIN_SEMANTIC_REVIEW.md](docs/EVALUATION_CHAIN_SEMANTIC_REVIEW.md) keeps `0003` vs `0007` clear and watches `0007` adjacency to `0006`. [SKILL_SUPPORT_SEMANTIC_REVIEW.md](docs/SKILL_SUPPORT_SEMANTIC_REVIEW.md) keeps [AOA-T-0015](techniques/evaluation/contract-test-design/TECHNIQUE.md) distinct from `0017`, but that seam remains the strongest evaluation watch point. |
| KAG canon pair | `AOA-T-0019`, `AOA-T-0021` | `clear` | [KAG_SOURCE_LIFT_SEMANTIC_REVIEW.md](docs/KAG_SOURCE_LIFT_SEMANTIC_REVIEW.md) keeps [AOA-T-0019](techniques/docs/frontmatter-metadata-spine/TECHNIQUE.md) narrow as the metadata spine and [AOA-T-0021](techniques/docs/bounded-relation-lift-for-kag/TECHNIQUE.md) narrow as one-hop direct relation hints. The active watch seams are schema creep and graph creep, not status weakness. |
...
[Promotion Readiness Matrix](mechanics/audit/parts/promotion-readiness-matrix/README.md), and
[External Evidence Sprint Runbook](mechanics/audit/parts/external-evidence-sprint-runbook/README.md)
...
- `keep`: [AGENT_WORKFLOWS_CORE_SEMANTIC_REVIEW.md](docs/AGENT_WORKFLOWS_CORE_SEMANTIC_REVIEW.md), [PUBLISHED_SUMMARY_SEMANTIC_REVIEW.md](docs/PUBLISHED_SUMMARY_SEMANTIC_REVIEW.md), [EVALUATION_CHAIN_SEMANTIC_REVIEW.md](docs/EVALUATION_CHAIN_SEMANTIC_REVIEW.md), [DOCS_BOUNDARY_SEMANTIC_REVIEW.md](docs/DOCS_BOUNDARY_SEMANTIC_REVIEW.md), [INTENT_CHAIN_SEMANTIC_REVIEW.md](docs/INTENT_CHAIN_SEMANTIC_REVIEW.md), [INSTRUCTION_SURFACE_SEMANTIC_REVIEW.md](docs/INSTRUCTION_SURFACE_SEMANTIC_REVIEW.md), [KAG_SOURCE_LIFT_SEMANTIC_REVIEW.md](docs/KAG_SOURCE_LIFT_SEMANTIC_REVIEW.md)
- `refresh later`: [SKILL_SUPPORT_SEMANTIC_REVIEW.md](docs/SKILL_SUPPORT_SEMANTIC_REVIEW.md) should remain the skill-support cluster anchor, but it is still the first candidate for a future wording refresh if `0015` vs `0017` or `0016` examples drift
- `expand`: none right now

Shadow review verdicts:

- `keep`: [PUBLISHED_SUMMARY_SHADOW_REVIEW.md](docs/PUBLISHED_SUMMARY_SHADOW_REVIEW.md) and [EVALUATION_CHAIN_SHADOW_REVIEW.md](docs/EVALUATION_CHAIN_SHADOW_REVIEW.md) are coherent and bounded
...
|---|---|---|
| `README -> START_HERE -> selection/index -> one technique` | `keep` | The shortest reader path is clear and bounded. [docs/START_HERE.md](docs/START_HERE.md) now does the right first-hop routing job. |
| `README -> START_HERE -> repo-doc surfaces -> release/governance` | `keep with route-friction note` | The path works, but [docs/README.md](docs/README.md) still duplicates some `START_HERE` routing instead of clearly behaving as the deeper map. |
| `START_HERE -> KAG/source-lift guides -> generated readers` | `keep` | [KAG_SOURCE_LIFT_GUIDE.md](docs/KAG_SOURCE_LIFT_GUIDE.md) is still the right contract anchor and keeps reader surfaces bounded. |
| `selection -> inspect -> expand -> object use` | `keep` | The runtime contract is explicit in [docs/START_HERE.md](docs/START_HERE.md) and still matches the generated selection surfaces. |
...
- `keep`: [TECHNIQUE_CAPSULE_GUIDE.md](docs/TECHNIQUE_CAPSULE_GUIDE.md), [REPO_DOC_SURFACE_LIFT_GUIDE.md](docs/REPO_DOC_SURFACE_LIFT_GUIDE.md), [TECHNIQUE_SHADOW_GUIDE.md](docs/TECHNIQUE_SHADOW_GUIDE.md), and several KAG guides already use the stable vocabulary of `authoritative source`, `reader companion`, `derived manifest`, and `what it must not become`.
- `repo-only closure`: [docs/README.md](docs/README.md) and [README.md](README.md) still describe some families in a more ad hoc route-map voice rather than the same stable contract vocabulary.
...
- [SELECTION_PATTERNS.md](docs/SELECTION_PATTERNS.md) stays bounded to direct relations, working sets, and common moves. No scoring, ranking, or multi-hop drift found.
- [SHADOW_PATTERNS.md](docs/SHADOW_PATTERNS.md) stays canonical-only and lookup-only. No policy-engine drift found.
- [REPO_DOC_SURFACES.md](docs/REPO_DOC_SURFACES.md) stays a bounded routing aid and does not claim deeper-guide authority.
- [KAG_SOURCE_LIFT_GUIDE.md](docs/KAG_SOURCE_LIFT_GUIDE.md) still explicitly defers graph inference, scoring, rationale layers, and policy routing.
