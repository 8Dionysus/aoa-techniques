...
This roadmap records future contour for the `aoa-techniques` RPG package. It
does not change technique status, quest status, role authority, progression
truth, runtime state, or validator behavior by itself.

## Current Contour

RPG is present as a candidate-only mechanics package for `aoa-techniques`.
It maps feat, unlock, quest-overlay, and owner-handoff reflection pressure while
keeping stronger owner authority outside this repo.
...
- A local RPG practice can leave mechanics only when it is small enough to stand
  as one technique bundle with stable inputs, outputs, contracts, risks,
