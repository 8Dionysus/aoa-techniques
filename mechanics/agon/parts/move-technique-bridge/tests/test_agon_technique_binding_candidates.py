...
def load_builder():
    path = PART_ROOT / "scripts" / "build_agon_technique_binding_candidates.py"
...
def test_agon_technique_binding_candidates_current() -> None:
    result = subprocess.run(
        [
            sys.executable,
            str(PART_ROOT / "scripts" / "build_agon_technique_binding_candidates.py"),
            "--check",
        ],
        cwd=REPO_ROOT,
        text=True,
...
def test_agon_technique_binding_candidate_shape() -> None:
    data = json.loads((PART_ROOT / "generated" / "agon_technique_binding_candidates.min.json").read_text())
    assert data["wave"] == "IV"
