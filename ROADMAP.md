...
| Next honest move | Run the direct-read singleton review for `tool-use/tool-gateway` before any twenty-eighth movement. |
...
Current latest tree review: the landed `practice-adoption-lifecycle` pilot
review validates the third automation split shelf, closes the rejected bulk
`automation-governance` tail with all nine IDs accounted, and chooses
