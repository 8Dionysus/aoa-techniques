...
- [AOA-T-0001](../techniques/agent-workflows/plan-diff-apply-verify-report/TECHNIQUE.md) - plan-diff-apply-verify-report (`agent-workflows`, current `workflow`): current-kind cues `workflow`, `plan`; opposing `guardrail` cues none. Seed family `agent-workflows-core` already spans both `workflow` and `guardrail`. Verdict: `keep current kind`.
