...
- [AOA-T-0085](../techniques/agent-workflows/multi-axis-quest-overlay/TECHNIQUE.md) - multi-axis-quest-overlay (`agent-workflows`, current `lift`): current-kind cues `overlay`; opposing `artifact` cues none. Seed family `donor-harvest` already spans both `artifact` and `lift`. Verdict: `keep current kind`.
