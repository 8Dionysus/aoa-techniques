...
class IncomingTopologyTestCase(unittest.TestCase):
    def test_packet_roots_are_evidence_only(self) -> None:
        packet_dirs = {
            path.name
        ...
        for packet in sorted(EXPECTED_PACKETS):
            manifest_path = INCOMING_ROOT / packet / "support" / "manifest.json"
    ...
    def test_no_packet_local_candidate_bundles_remain(self) -> None:
        candidate_bundle_paths = list(INCOMING_ROOT.glob("**/candidate_bundles/**"))
    ...
    def test_registries_do_not_claim_live_seed_bundles(self) -> None:
        ...
        for registry_path in sorted(INCOMING_ROOT.glob("*/support/registry.json")):
    ...
    def test_incoming_root_names_closed_packet_stop_line(self) -> None:
        ...
        self.assertIn("closed-no-import", text)
        self.assertIn("new Distillation intake", text)
        self.assertIn("Do not recreate packet-local `candidate_bundles/`", text)

    def test_no_live_debt_markers_remain_in_current_incoming_docs(self) -> None:
        current_files = [
            path
