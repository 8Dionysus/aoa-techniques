...
`aoa-techniques` exists so reusable practice can mature without collapsing into
larger execution or governance systems.

It should:

- keep each technique as one atomic executable move
- preserve standalone portability for external builders
- keep AoA organ fidelity through IDs, provenance, mechanics, generated
  surfaces, and owner boundaries
- support `1000+` well-classified techniques as an early scale target
- keep small-agent execution possible after orchestration selects and packs the
  right context
- let skills, evals, routing, playbooks, agents, memory, KAG, stats, runtime,
  and adjacent repos consume technique truth without owning it
...
| derived graph, retrieval, or KAG substrate semantics | `aoa-kag`, derived from source owners |
| observability summaries or movement windows | `aoa-stats`, derived from owner-local evidence |
| runtime, services, storage, infrastructure, and deployment | `abyss-stack` |
| AoA constitutional identity, federation map, and center law | `Agents-of-Abyss` |
| ToS-authored knowledge meaning | `Tree-of-Sophia` |

## Canon Discipline

A technique claim is healthy when a reader can identify the move, inputs,
outputs, stop line, validation signal, risk, and owner boundary without origin
lore.

AoA references may provide provenance, deeper context, or integration routes.
They must not become hidden dependencies for using the technique outside OS
Abyss.

Mechanics may prepare canon, preserve lineage, and route candidates. They do
not replace technique bundles, change technique status by themselves, or turn
candidate pressure into public canon before the atom and topology contracts are
met.

Generated outputs are companions. They may route, compress, or expose technique
meaning for machines, but authored sources keep authority.

## Review Rule

Before changing the repository's root posture, technique-canon boundary, or
