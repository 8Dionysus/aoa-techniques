# Codex Handoff - Chat Graph Review Mailbox

You are operating inside `aoa-techniques` under explicit human control.

## Your job

Use the staging surfaces in `incoming/chat-graph-review-mailbox/` to prepare candidate bundles and narrowing memos for the active Wave 2 candidates.

## Read first

1. `README.md` at repo root
2. `mechanics/distillation/parts/donor-refinery/README.md`
3. `mechanics/distillation/parts/external-import-runbook/README.md`
4. `TECHNIQUE_INDEX.md`
5. `incoming/chat-graph-review-mailbox/docs/EXTERNAL_TECHNIQUE_CANDIDATES_CHAT_GRAPH_REVIEW_MAILBOX.md`
6. `incoming/chat-graph-review-mailbox/docs/CHAT_GRAPH_REVIEW_MAILBOX_PLANTING_ORDER.md`

## Operating posture

- treat this wave as staging soil, not merge authority
- keep bundle work inside `candidate_bundles/**`
- do not edit `TECHNIQUE_INDEX.md`, `generated/**`, or repo-wide queue docs
- keep `phase-synchronized-agent-handoff` outside this wave

## Desired worker output

Always answer in this shape:

- candidate chosen
- overlap watch
- boundary statement
- what stays out
- files touched or proposed
- whether operator approval is needed
